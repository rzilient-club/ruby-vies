...
RSpec.describe Rz::Vies do
  it "has a version number" do
    expect(Rz::Vies::VERSION).not_to be nil
  end

  describe "Call Vies using a invalid VAT" do
    it "checks the VAT number and returns an error" do
      resp = Rz::Vies::Client.new.check_vat_details({ country_code: "FR", vat_number: "0" })
    ...
    it "checks a valid VAT number" do
      resp = Rz::Vies::Client.new.check_vat_details({ country_code: "FR", vat_number: "881969976" })
    ...
    it "checks the VAT number doesn't belong to EU country" do
      resp = Rz::Vies::Client.new.check_vat_details({ country_code: "GB", vat_number: "0" })
